...
const evidence = { observedAt: new Date().toISOString(), apiBase, product: contract.product, routes: [] };
...
async function probe(route) {
  const observations = [];
  for (let request = 1; request <= route.allowance + 1; request += 1) {
    const token = `focus-resume-release-probe-${Date.now()}-${request}`;
  ...
  requireCondition(allowed.every((result) => result.status === route.normalStatus), `${route.id}: initial responses were ${allowed.map(({ status }) => status).join(', ')}`);
  requireCondition(blocked?.status === contract.response.status, `${route.id}: request ${route.allowance + 1} must return 429, received ${blocked?.status}`);
  requireCondition(validRetryAfter(blocked.retryAfter, contract.windowSeconds), `${route.id}: 429 must include Retry-After from 1 to ${contract.windowSeconds}, received ${blocked.retryAfter ?? 'none'}`);
  ...
  if (route.id === 'checkout') requireCondition(!blocked.location, 'checkout: limited request created a checkout redirect');
}
...
try {
  ...
  console.log(`gateway rate limit: ${contract.routes.map((route) => `${route.id} ${route.allowance}/${contract.windowSeconds}s`).join(', ')} enforced`);
} finally {
